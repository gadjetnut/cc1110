...
__xdata static const char data[DATA_AMOUNT] = "Flash Controller";
...
int send_to_serial (void)
     ...
     * Transfer UART data
     */
    uart0StartTxDmaChan(&uartDmaRxTxCh[1], 1, writeCheck, DATA_AMOUNT-1);
    return(0);
}
...
int main(void)
{
    ...
    dma0_config.SRCADDRH  = (((uint16_t)(__xdata uint16_t *)data) >> 8) & 0x00FF;
    dma0_config.SRCADDRL   = ((uint16_t)(__xdata uint16_t *)data) & 0x00FF;
    dma0_config.DESTADDRH  = (FLASH_FWDATA_ADDR >> 8) & 0x00FF;
    dma0_config.DESTADDRL   = FLASH_FWDATA_ADDR & 0x00FF;
    dma0_config.LENH  = (DATA_AMOUNT >> 8) & 0x00FF;
    dma0_config.VLEN      = DMA_VLEN_USE_LEN;
    dma0_config.LENL   = DATA_AMOUNT & 0x00FF;
