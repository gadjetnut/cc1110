...
static uint16_t __xdata uartTxIndex;
static uint16_t __xdata uartRxIndex;

// Variable for buffer indexing
static uint16_t __xdata i;

// Variable for UART packet monitoring
static uint8_t __xdata uartPktReceived = 1;
...
void uart0StartRxDmaChan( DMA_DESC *uartDmaRxDescr,
                          uint8_t uartDmaRxChan,
                          uint8_t* uartRxBuf,
                          uint16_t uartRxBufSize);
...
int main (void)
{
  ...
  while(1)
  {
    ...
    while(1)
    {
      memcpy(uartTxBuffer, "Hello World!",12);
      uart0StartTxDmaChan(&uartDmaRxTxCh[1], 1, uartTxBuffer, 12);
      P1_3^=1;
      delayms(1000);
    }
    #endif
...
*              uint8_t uartDmaRxChan - DMA channel number for UART RX
*              uint8_t* uartRxBuf - pointer to allocated UART RX buffer
*              uint16_t uartRxBufSize - size of allocated UART RX buffer
*
* @return      0
*/
void uart0StartRxDmaChan( DMA_DESC *uartDmaRxDescr,
                          ...
                          uint16_t uartRxBufSize)
{
  ...
  uartDmaRxDescr->DESTADDRH  = (uint16_t)uartRxBuf>>8;
  uartDmaRxDescr->DESTADDRL  = (uint16_t)uartRxBuf;
  uartDmaRxDescr->SRCADDRH   = ((uint16_t)(&X_U0DBUF) >> 8) & 0x00FF;
  uartDmaRxDescr->SRCADDRL   = (uint16_t)(&X_U0DBUF) & 0x00FF;
  ...
  if (uartDmaRxChan < 1)
  {
    DMA0CFGH = (uint8_t)((uint16_t)uartDmaRxDescr>>8);
    DMA0CFGL = (uint8_t)((uint16_t)uartDmaRxDescr&0x00FF);
  } else {
    DMA1CFGH = (uint8_t)((uint16_t)uartDmaRxDescr>>8);
    DMA1CFGL = (uint8_t)((uint16_t)uartDmaRxDescr&0x00FF);
  }
