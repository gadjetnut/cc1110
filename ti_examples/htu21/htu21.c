...
uint8_t sht21_WriteByte(uint8_t txByte)
{
    uint8_t __far mask, error = 0;
    for (mask = 0x80; mask > 0; mask >>= 1) //shift bit for masking (8 times)
    {
        if ((mask & txByte) == 0)
            set_sda_low(); //masking txByte, write bit to SDA-Line
        else
            set_sda_high();
        delayMicro(1);  //data set-up time (t_SU;DAT)
        set_scl_high(); //generate clock pulse on SCL
        delayMicro(5);  //SCL high time (t_HIGH)
        set_scl_low();
        delayMicro(1); //data hold time(t_HD;DAT)
    }
    set_sda_high(); //release SDA-line
    set_scl_high(); //clk #9 for ack
    delayMicro(1);  //data set-up time (t_SU;DAT)
    if (get_sda_conf() == 1)
        error = 0x01; //check ack from i2c slave
    set_scl_low();
    delayMicro(20); //wait time to see byte package on scope
    return error;   //return error code
}

uint8_t sht21_ReadByte(etI2cAck ack)
{
    uint8_t __far mask, rxByte = 0;
    set_sda_high();                         //release SDA-line
    for (mask = 0x80; mask > 0; mask >>= 1) //shift bit for masking (8 times)
    {
        set_scl_high(); //start clock on SCL-line
        delayMicro(1);  //data set-up time (t_SU;DAT)
    ...
    sda_ack(ack);   //send acknowledge if necessary
    delayMicro(1);  //data set-up time (t_SU;DAT)
    set_scl_high(); //clk #9 for ack
    delayMicro(5);  //SCL high time (t_HIGH)
    set_scl_low();
...

{
    ...
    sht21_StartCondition();
    error |= sht21_WriteByte(sht21_ADR_W); // I2C Adr
    switch (eSHT2xMeasureType)
    {
    case HUMIDITY:
        error |= sht21_WriteByte(TRIG_RH_MEASUREMENT_HM);
        break;
    case TEMP:
        error |= sht21_WriteByte(TRIG_T_MEASUREMENT_HM);
        break;
    }
    //-- wait until hold master is released --
    sht21_StartCondition();
    error |= sht21_WriteByte(sht21_ADR_R);
    set_scl_high();            // set SCL I/O port as input
    ...
    data[0] = sht21_ReadByte(ACK);
    data[1] = sht21_ReadByte(ACK);
    *pMeasurand = data[1] | data[0] << 8;
    checksum = sht21_ReadByte(NO_ACK);
    //-- verify checksum --
...
uint8_t SHT21_MeasurePoll(etSHT2xMeasureType eSHT2xMeasureType, uint16_t __far *pMeasurand)
{
    ...
    {
    case HUMIDITY:
        error |= sht21_WriteByte(TRIG_RH_MEASUREMENT_POLL);
        break;
    case TEMP:
        error |= sht21_WriteByte(TRIG_T_MEASUREMENT_POLL);
        break;
    }
    ...
    do
    {
        ...
        if (i++ >= 20)
            break;
    } while (sht21_WriteByte(sht21_ADR_R) == ACK_ERROR);
    if (i >= 20)
        error |= TIME_OUT_ERROR;
    //-- read two data bytes and one checksum byte --
    data[0] = sht21_ReadByte(ACK);
