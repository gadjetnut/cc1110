...
/***********************************************************************************
  Filename:     spi_ex1_slave.c

  Description:  This example uses a slave to send data to a master using SPI.

  Comments:     To execute this example, use spi_ex1_master.c in IAR project
                spi1_master_cc2510, as the master. The slave's code must be
                executed before executing the master's code, since the slave
                is clocked by the master. The bytes sent are simply numbers
                0x00 up to BUFFER_SIZE. Note that if BUFFER_SIZE is larger than
                0xFF, than the element 0xFF in the array will have the value
                0x00 and so on. When bytes up to BUFFER_SIZE are sent, the
                example will end and the red LED on SMARTRF04EB will be set.
